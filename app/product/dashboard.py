...
from app.pipeline.attributes import ingest_records, parse_attributes, prepare_records


def prepare_dashboard_records(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Backward-compatible Streamlit wrapper around shared backend normalization."""
    return prepare_records(records)


def ingest_dashboard_records(registry: Any, records: list[dict[str, Any]]) -> Any:
    """Backward-compatible Streamlit wrapper around shared backend ingestion."""
    return ingest_records(registry, records)
